...
function App() {
  ...
  const [budgetsByMonth, setBudgetsByMonth] = useState({
    ...
    "2025-04": [{ id: 1, name: "Makanan", budgets: 0, used: 0, icon: null }],
  });
  ...
  const [AkunByMonth, setAkunByMonth] = useState({
    ...
    "2025-04": [{ id: 1, name: "Bank BRI", Total: 0, used: 0, icon: null }],
  });
    ...
    if (modalType === "budget") {
      ...
      setAkunByMonth((prev) => {
        ...
        if (oldItem?.akunDipakai) {
          for (const [akunName, jumlah] of Object.entries(
            oldItem.akunDipakai
          )) {
    ...
    setBudgetsByMonth((prev) => {
      ...
      const updated = current.map((budget) => {
        const totalUsed = transactions
          .filter(
            (tx) =>
              tx.category === budget.name && tx.date.startsWith(selectedMonth)
          )
  ...
  useEffect(() => {
    setAkunByMonth((prev) => {
      ...
      const updated = current.map((akun) => {
        ...
        const totalAmount = pendapatanTx.reduce(
        ...
        const usedAmount =
          budgetsByMonth[selectedMonthAkun]?.reduce((sum, kategori) => {
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-wrap gap-3 py-1 w-full">
              {displayedBudgets.map((item) => {
                const percentage =
                  item.budgets > 0
                    ? ((item.used || 0) / item.budgets) * 100
                    : 0;

                return (
                  <div key={item.id} className="w-4/6 sm:w-4/6 lg:w-2/3">
