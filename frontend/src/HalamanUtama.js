...
function App() {
    ...
    if (savedMonths) {
      try {
        ...
        if (
          ...
          const d = new Date();
          const currentMonth = `${d.getFullYear()}-${String(
            d.getMonth() + 1
          ).padStart(2, "0")}`;
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const budgetDataFiltered = budgetDataRaw.filter(
        (item) => parseFloat(item.amount) > 0 || parseFloat(item.used) > 0
      );
      ...
      setBudgetsByMonth((prev) => {
        ...
        console.log(
          `[FRONTEND] Budgets for ${selectedMonth} (filtered):`,
          budgetDataFiltered
        );
      ...
      if (budgetDataFiltered.length === 0) {
        setCategories([]);
        console.log(
          `[FRONTEND] No active budgets found for ${selectedMonth}, setting categories to empty.`
        );
      } else {
        ...
        console.log(
          `[FRONTEND] Active Categories for ${selectedMonth} derived from filtered budgets:`,
  ...
  const handleAddMonth = async (newMonth) => {
    ...
    try {
      ...
      setAvailableMonths((prevMonths) => {
        const updated = [...new Set([...prevMonths, newMonth])].sort(
      ...
      showNotification(
        "Berhasil",
        "Bulan baru berhasil ditambahkan! Anda bisa mulai menambahkan kategori dan budget untuk bulan ini.",
        "success"
      );
    } catch (error) {
      ...
      showNotification(
        "Error",
        "Terjadi kesalahan saat menambah bulan baru: " + error.message,
        "error"
      );
    }
  };
    ...
    showConfirmation(
      ...
      async () => {
        ...
        try {
          ...
          showNotification(
            "Berhasil",
            "Kategori berhasil dihapus dari semua bulan. Sisa dana budget telah dikembalikan ke akun asalnya.",
            "success"
          );
        } catch (error) {
  ...
  return (
    <div className="flex-col items-start justify-center min-h-screen bg-gray-200 font-sans">
      <div className="w-full bg-gray-700 shadow-md text-center py-9 md:py-32 lg:py-32 relative">
        {" "}
        {/* Added relative for absolute positioning of logout */}
      ...
      <div className="flex justify-center mt-5">
        <div className="inline-flex rounded-full overflow-hidden shadow-md border border-gray-500">
          <button className="bg-gray-500 text-white px-6 py-2 text-sm md:text-base font-medium rounded-l-full">
            Transaksi
          </button>
          <button
      ...
      <div className="py-10 px-10 flex flex-col md:flex-row bg-gray-200">
        {" "}
        {/* Changed to flex-col for mobile, md:flex-row for larger screens */}
        <div className="flex flex-col w-full md:w-1/6 md:mr-8 mb-8 md:mb-0">
          {" "}
          {/* Adjusted width and margin for responsiveness */}
          <div className="flex flex-col">
            <h1 className="bg-gray-500 rounded-md text-[13px] md:text-base lg:text-lg font-bold text-white px-2 py-1 w-full md:w-3/4 mb-2">
              {" "}
          ...
          <h1 className="bg-gray-500 rounded-md text-[13px] md:text-base lg:text-lg font-bold text-white px-2 py-1 w-full md:w-3/4 mt-8 mb-2">
            {" "}
          ...
          <div className="flex flex-wrap gap-3 py-1 w-full">
            {accounts.length > 0 ? (
              accounts.map((account) => {
                console.log(
                  `[FRONTEND Render] Account ID: ${account.id}, Name: ${account.name}, Balance: ${account.balance}`
                );
                return (
                  <div
                    key={account.id}
                    className="w-full sm:w-1/2 md:w-full lg:w-2/3"
                  >
                    {" "}
                    {/* Adjusted width for responsiveness */}
        ...
        <div className="w-full md:w-3/6 px-0 md:px-2">
          ...
          <div className="flex flex-col">
            {/* Title + Table */}
            <h1 className="bg-gray-300 rounded-md text-lg md:text-2xl lg:text-3xl font-bold text-gray-800 mx-0 md:mx-2 px-2 py-1 w-full mb-2">
              {" "}
              {/* Adjusted width and margin */}
              Transaksi
            </h1>
            ...
            <div className="border-b border-gray-200 dark:border-gray-700 mb-2">
              <ul className="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400">
                ...
                <button
                  onClick={() =>
                    handleOpenEditTransactionModal({ type: "expense" })
                  }
          ...
          <div className="flex flex-col mt-10">
            ...
            <div className="border-b border-gray-200 dark:border-gray-700 mb-2">
              <ul className="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400">
                ...
                <button
                  onClick={() =>
                    handleOpenEditTransactionModal({ type: "income" })
                  }
        ...
        {/* Bagian Kanan */}
        <div className="w-full md:w-2/6 px-0 md:px-2 mt-8 md:mt-0">
          {" "}
          {/* Adjusted width and padding for responsiveness */}
          <div className="flex flex-col">
            <h1 className="bg-gray-300 rounded-md text-[13px] md:text-base lg:text-lg font-bold text-gray-800 mx-0 md:mx-2 px-2 py-1 w-full mb-2">
              {" "}
              {/* Adjusted width and margin */}
              Graph Pengeluaran
            </h1>
